...
import { buildSlackAuthorisationUrl } from "@/lib/context/slack-api";
...
export async function GET(request: NextRequest) {
  try {
    ...
    const redirectUri = new URL("/api/context/slack/callback", siteConfig.url).toString();

    return NextResponse.redirect(buildSlackAuthorisationUrl(state, redirectUri));
  } catch (error) {
    ...
    if (message.includes("not configured")) {
      return errorResponse("Slack connection is not configured", 503);
    }
    console.error("Slack connect failed", error);
    return errorResponse("Could not start Slack connection", 500);
  }
}
