...
import { buildGmailContextAuthorisationUrl } from "@/lib/context/gmail-api";
...
export async function GET(request: NextRequest) {
  try {
    ...
    const returnTo = requestedReturnTo?.startsWith("/") === true
      ? requestedReturnTo
      : "/";
    ...
    return NextResponse.redirect(buildGmailContextAuthorisationUrl(state));
  } catch (error) {
    ...
    if (message.includes("not configured")) {
      return errorResponse("Gmail connection is not configured", 503);
    }
    console.error("Gmail context connect failed", error);
    return errorResponse("Could not start Gmail connection", 500);
  }
}
